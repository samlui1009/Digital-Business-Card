import './AboutMe.css';

function AboutMe() {
    return (
        <>
            <div className="about-me">
                <div className="personal-info">
                    <h1 className="full-name">Sam Lui</h1>
                    <h4 className="tagline">UBC BCS | CSMLS Certified | SFU HealthSci Alumni</h4>
                </div>
                <h3>About:</h3>
                <p>As a former healthcare and genetics professional pivoting into the technology sector, I bring
                    a unique skillset that encompasses technical aptitude, a penchant for solving complex problems, 
                    and a passion for continued learning. I hope to one day make monumental changes and lasting positive 
                    impact to the digital health space. 
                </p>
            </div>
        </>
    )
}

export default AboutMe